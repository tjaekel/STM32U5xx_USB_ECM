...
UINT        temp_realm_length = 0;
    ...
    length =  0;
#ifdef FLASH_PAGES
    fxs_directory_information_get(server_ptr -> nx_web_http_server_media_ptr, server_ptr -> nx_web_http_server_request_resource, FX_NULL,
    ...
    if(status)
    {
        ...
        /* Close the file.  */
#ifdef FLASH_PAGES
        fxs_file_close(&(server_ptr -> nx_web_http_server_file));
#else
...
#ifdef FLASH_PAGES
        status =  fxs_file_read(&(server_ptr -> nx_web_http_server_file), new_packet_ptr -> nx_packet_append_ptr,
                                                temp, &temp);
#else
